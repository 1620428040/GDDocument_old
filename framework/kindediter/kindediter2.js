...
K.options = {
	...
	items : [
		...
		'flash', 'media', 'insertfile', 'addiframe', 'table', 'hr', 'emoticons', 'baidumap', 'pagebreak',
...
KindEditor.lang({
	...
	'addiframe.url': 'URL',
	'addiframe.title': '文件说明',
	'addiframe.upload': '上传',
	'addiframe.viewServer': '文件空间',
	'table.cells' : '单元格数',
	'table.rows' : '行数',
	'table.cols' : '列数',
	'table.size' : '大小',
...
* 点击插入PDF
*******************************************************************************/
KindEditor.plugin('addiframe', function (K) {
    var self = this, name = 'addiframe',
		allowFileUpload = K.undef(self.allowFileUpload, true),
		allowFileManager = K.undef(self.allowFileManager, false),
		formatUploadUrl = K.undef(self.formatUploadUrl, true),
		uploadJson = K.undef(self.uploadJson, self.basePath + 'php/upload_json.php'),
    ...
    self.clickToolbar(name, function () {
        self.plugin.fileDialog({
            clickFn: function (url, title) {
                var html = '<iframe width="100%" height="600px" src="' + url + '"></iframe>';
                self.insertHtml(html).hideDialog().focus();
            }
        });
    });
